...
public class TaskStateInDBManager {
    ...
    boolean DdlAvailable(String tablename) {
        String selectQuery = "SELECT * FROM " + TaskStatusTable.get(tablename) + " ORDER BY taskid DESC LIMIT :limit";
        NativeQuery query = session.createNativeQuery(selectQuery);
        query.setParameter("limit", this.dmlPartitionNum);
        List<Object[]> rows = query.list();
    ...
    void setAllTaskStatus(String tablename) {
        // UPDATE Query
        String upsertQuery = "UPDATE " + TaskStatusTable.get(tablename) + " SET status = 0 WHERE status = 2";
        NativeQuery query = session.createNativeQuery(upsertQuery);
        query.executeUpdate();
