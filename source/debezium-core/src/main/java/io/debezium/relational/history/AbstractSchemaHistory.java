...
public abstract class AbstractSchemaHistory implements SchemaHistory {
    ...
    public final void record(Map<String, ?> source, Map<String, ?> position, String databaseName, String ddl)
        ...
        record(source, position, databaseName, null, ddl, null, Clock.SYSTEM.currentTimeAsInstant(), null);
    }

    @Override
    public final void record(Map<String, ?> source, Map<String, ?> position, String databaseName, String schemaName,
                             String ddl, TableChanges changes, Instant timestamp, String ddlVersion)
            throws SchemaHistoryException {
        final HistoryRecord record = new HistoryRecord(source, position, databaseName, schemaName, ddl, changes, timestamp, ddlVersion);
        storeRecord(record);
        DDLVersionInfo.getInstance().storeDDLVersionMap();
    ...
    public void recover(Map<Map<String, ?>, Map<String, ?>> offsets, Tables schema, DdlParser ddlParser) {
        ...
        offsets.forEach((Map<String, ?> source, Map<String, ?> position) -> {
            ...
            stopPoints.put(srcDocument, new HistoryRecord(source, position, null, null, null, null, null, null));
        });
