...
public class SchemaChangeEvent {
    ...
    //DDL 레코드 : DDLVersion의 TableName
    public String getTableInstance() {
        String tableInstance = source.getString("table");

        //pt-osc
        if (tableInstance != null){
            //RENAME TABLE, tableName 값이 , 로 두개가 들어간 경우 고려
            tableInstance = tableInstance.split(",")[0];

            //_new,_old 테이블 기존 테이블과 동일한 tableName
        ...
        //CREATE INDEX, DROP INDEX tableName
        if ((tableInstance == null || tableInstance.equals("")) && ddl.toUpperCase().contains("INDEX")) {
            String[] tokens = ddl.split("\\s+");

            for (int i = 0; i < tokens.length; i++) {
                if (tokens[i].equalsIgnoreCase("ON") && i + 1 < tokens.length) {
                    tableInstance = tokens[i + 1];
                    break;
                }
            }
        }
    ...
    //QueryID : RENAME TABLE gtid 동일한 경우
    public String getSpecificId() {
        try {
            MessageDigest md = MessageDigest.getInstance("MD5");
            byte[] messageDigest = md.digest(ddl.getBytes());
